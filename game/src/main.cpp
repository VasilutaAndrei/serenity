...
int main(int argc, char **argv)
{
	...
	while (!exppet_table_open.eof())
	{
		...
		if (exppet_table_common[exppet_table_counter] < 2147483647) {
			sys_log(0, "Valoare depasita in exppettable.txt: %d !", exppet_table_common[exppet_table_counter]);
			exppet_table_counter++;
		}
		else {
			fprintf(stderr, "[MAIN] Nu gasesti fisierul exppettable.txt in /usr/game/share/\n");
			break;
		}
	}
#endif	
