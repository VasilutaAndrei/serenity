...
#ifdef ENABLE_SPECIAL_STORAGE	// Crash piss of shit
			else if (IsUpgradeItem() && m_bWindow == UPGRADE_INVENTORY)
			{
				if (m_wCell >= SPECIAL_INVENTORY_MAX_NUM)
					sys_err("CItem::RemoveFromCharacter: pos >= SPECIAL_INVENTORY_MAX_NUM IsUpgradeItem");
				else
					pOwner->SetItem(TItemPos(UPGRADE_INVENTORY, m_wCell), NULL);
			}
			else if (IsBook() && m_bWindow == BOOK_INVENTORY)
			{
				if (m_wCell >= SPECIAL_INVENTORY_MAX_NUM)
					sys_err("CItem::RemoveFromCharacter: pos >= SPECIAL_INVENTORY_MAX_NUM IsBook");
				else
					pOwner->SetItem(TItemPos(BOOK_INVENTORY, m_wCell), NULL);
			}
			else if (IsStone() && m_bWindow == STONE_INVENTORY)
			{
				if (m_wCell >= SPECIAL_INVENTORY_MAX_NUM)
					sys_err("CItem::RemoveFromCharacter: pos >= SPECIAL_INVENTORY_MAX_NUM IsStone");
				else
					pOwner->SetItem(TItemPos(STONE_INVENTORY, m_wCell), NULL);
			}
#endif
			else
			{
...
int CItem::FindEquipCell(LPCHARACTER ch, int iCandidateCell)
{
	...
	if ((0 == GetWearFlag() || ITEM_TOTEM == GetType()) && ITEM_COSTUME != GetType() && ITEM_DS != GetType() && ITEM_SPECIAL_DS != GetType() && ITEM_RING != GetType() && ITEM_BELT != GetType() && ITEM_NORMAL_PET != GetType() && ITEM_NORMAL_BUFF != GetType() && ITEM_NORMAL_TITLE != GetType())
#else 
	...
	else if (GetType() == ITEM_COSTUME)
	{
...
#endif
		else if (GetSubType() == USE_RING)
		{
			if (ch->GetWear(WEAR_COSTUME_RING1))
				return WEAR_COSTUME_RING2;
			else
				return WEAR_COSTUME_RING1;
		}
	}
		...
		return WEAR_BELT;
#ifdef ENABLE_PET_SLOT
	else if (GetType() == ITEM_NORMAL_PET)
		return WEAR_NORMAL_PET;
	else if (GetType() == ITEM_NORMAL_BUFF)
		return WEAR_NORMAL_BUFF;
...
void CItem::ForceSave(LPCHARACTER ch_ptr)
{
	if(ch_ptr)
	{
...
{
	if (GetType() == ITEM_NORMAL_PET)
		return true;
	
	return false;
}
bool CItem::IsBuffItem()
{
	if (GetType() == ITEM_NORMAL_BUFF)
		return true;
