...
EVENTFUNC(pvp_duel_counter)
{
    ...
    switch (info->state)
    {
		case 0:
		{
			...
			if (chB == NULL || chA == NULL)
			{
				sys_err("Duel: Duel start event info is null.##1");
				return 0;
			}			
			break;
		}
		...
		case 2:
		{
			...
			{
#ifdef ENABLE_MOUNT_COSTUME_SYSTEM					
				for (int mountIndex = 20110; mountIndex < 20299; mountIndex++)
				{
					CMountSystem* chMount = chA->GetMountSystem(); 
					CMountSystem* victimMount = chB->GetMountSystem();
						
					CMountActor* chMountActor = chMount->GetByVnum(mountIndex);
					CMountActor* victimMountActor = victimMount->GetByVnum(mountIndex);	
						
					if (chMountActor != NULL)
						chMount->Unsummon(mountIndex); 	
							
					if (victimMountActor != NULL)
						victimMount->Unsummon(mountIndex); 
				}
#endif		
			}				
			...
			if ((chA->GetDuel("BlockBuff")) && (chB->GetDuel("BlockBuff")))
			{
				...
				for (int petIndex = 34001; petIndex < 34150; petIndex++)
				{
					CSupportSystem* chSupp = chA->GetSupportSystem(); 
					CSupportSystem* victimSupp = chB->GetSupportSystem();
						
					CSupportActor* chSupportActor = chSupp->GetByVnum(petIndex);
					CSupportActor* victimSupportActor = victimSupp->GetByVnum(petIndex);	
						
					if (chSupportActor != NULL)
						chSupp->Unsummon(petIndex); 	
							
					if (victimSupportActor != NULL)
						victimSupp->Unsummon(petIndex); 
				}
#endif			
			}
		...
		case 4:
		{
			if (chB == NULL || chA == NULL)//astea eu le-am pus aseara
			{
				sys_err("Duel: Duel start event info is null.##1");
				break;
			}	
...
EVENTFUNC(pvp_check_disconnect)
{
	...
	//if (chA && chB) //done duel_test_
	//{
	//	chA->ChatPacket(CHAT_TYPE_INFO, "spammm 1");		
	//	chB->ChatPacket(CHAT_TYPE_INFO, "spammm 2");			
	//}
	...
	while (it2 != it->second.end())
	{
		...
		if (dwCompanionPID == dwKillerPID)
		{
			if (pkPVP->IsFight())
			{
				...
				event_cancel(&pkPVP->m_pCheckDisconnect);
				pkPVP->m_pCheckDisconnect = NULL;	
#else
...
#ifdef ENABLE_ANTI_WAITHACK_DMG
	if (pkChr->GetWaitHackCounter() == 1) {
