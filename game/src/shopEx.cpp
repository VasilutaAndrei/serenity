...
bool CShopEx::AddGuest(LPCHARACTER ch,DWORD owner_vid, bool bOtherEmpire)
{
	...
	pack2.usablePoints = ch->GetQuestFlag("battle_zone.usable_points");
	pack2.limitMaxPoints = quest::CQuestManager::instance().GetEventFlag("battle_limit_max_points");
#endif
...
long long CShopEx::Buy(LPCHARACTER ch, BYTE pos)
{
	...
	case SHOP_COIN_TYPE_BATTLE_POINT:
		{
			if (ch->GetBattlePoints() < (long) dwPrice)
			{
				sys_log(1, "ShopEx::Buy : Not enough battle points : %s has %d, price %d", ch->GetName(), ch->GetBattlePoints(), dwPrice);
				return SHOP_SUBHEADER_GC_NOT_ENOUGH_BATTLE_POINTS;
			}
			else if (ch->GetQuestFlag("battle_zone.usable_points") < (long) dwPrice)
			{
				sys_log(1, "ShopEx::Buy : Exceed limit today : %s has %d, price %d", ch->GetName(), ch->GetQuestFlag("battle_zone.usable_points"), dwPrice);
	...
	case SHOP_COIN_TYPE_BATTLE_POINT:
		{
			ch->PointChange(POINT_BATTLE_POINT, -dwPrice, false);
			ch->SetQuestFlag("battle_zone.usable_points", ch->GetQuestFlag("battle_zone.usable_points")-dwPrice);
			ch->ChatPacket(CHAT_TYPE_COMMAND, "refresh_used_bp %d" , ch->GetQuestFlag("battle_zone.usable_points"));
		}
		break;
#endif
	}
