...
namespace MeleyLair
{
	...
	bool CMgrMap::Damage(LPCHARACTER pkStatue)
	{
		...
		if (bStep == 1)
			...
			iHPCalc = pkStatue->GetMaxHP() * 25 / 100;
	...
	bool CMgr::Enter(CGuild* pkGuild, LPCHARACTER pkChar, int & iLimit)
	{
		...
		if ((pkGuild) && (pkChar))
		{
			...
			std::auto_ptr<SQLMsg> pMsg(DBManager::instance().DirectQuery("SELECT id FROM player.guild WHERE dungeon_start = %u AND dungeon_ch = %u", dwStart, dwCH1));
			...
			std::auto_ptr<SQLMsg> pMsg2(DBManager::instance().DirectQuery("SELECT id FROM player.guild WHERE dungeon_start = %u AND dungeon_ch = %u", dwStart, dwCH2));
			...
			std::auto_ptr<SQLMsg> pMsg3(DBManager::instance().DirectQuery("SELECT id FROM player.guild WHERE dungeon_start = %u AND dungeon_ch = %u", dwStart, dwCH3));
			...
			std::auto_ptr<SQLMsg> pMsg4(DBManager::instance().DirectQuery("SELECT id FROM player.guild WHERE dungeon_start = %u AND dungeon_ch = %u", dwStart, dwCH4));
