		...
		if (iDamage > 0)
		{
			...
			if (ch)
			{
				...
				if(dwCount)
				{
					...
					ch->SetQuestFlag(flagNames[6919 - GetRaceNum()], ch->GetQuestFlag(flagNames[6919 - GetRaceNum()]) + dwCount);
...
#ifdef ENABLE_ATTENDANCE_EVENT
		if(GetRaceNum() >= 6915 && GetRaceNum() <= 6919)
		{
			RewardAttendance();
		}
#endif		
...
void CHARACTER::Dead(LPCHARACTER pkKiller, bool bImmediateDead)
{
	...
	if (true == isBattle)
	{
		CBattleZone::instance().PlayerKill( this, pkKiller );
	}
#endif
	...
	if (pkKiller && IsPC())
	{
		...
		else
		{
			...
			{
#ifdef __ENABLE_BATTLE_FIELD__
				if (!isAgreedPVP && !isUnderGuildWar && !IsKillerMode() && GetAlignment() >= 0 && !isDuel && !isForked && !isBattle)
#else
...
bool CHARACTER::Damage(LPCHARACTER pAttacker, int dam, EDamageType type) // returns true if dead
{
	...
	if (pAttacker)
	{
		...
		float tempDam = dam;
#ifdef ENABLE_ATTENDANCE_EVENT
		dam = (GetRaceNum() >= 6915 && GetRaceNum() <= 6919) ? 1 : tempDam * damMul + 0.5f;
#else
...
static void GiveExp(LPCHARACTER from, LPCHARACTER to, int iExp)
{
	...
	if (to->GetNewPetSystem()) {
		if (to->GetNewPetSystem()->GetLevel() < gPlayerMaxLevel)
		{
			if (to->GetNewPetSystem()->IsActivePet())
			{
