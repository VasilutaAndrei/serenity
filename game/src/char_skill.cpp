...
// #endif
	...
	if(IsSupport())
		pkSk->SetPointVar("iq", GetSupportInt());
	else
		pkSk->SetPointVar("iq", GetPoint(POINT_IQ));
#else
	pkSk->SetPointVar("iq", GetPoint(POINT_IQ));
...
int __MountAtkSkill(DWORD dwMountVnum, bool IsNew) // updated to 2014/12/10
{
	if (!dwMountVnum)
...
bool CHARACTER::CanUseSkill(DWORD dwSkillVnum) const
{
	...
	if (true == IsRiding())
	{
		...
		int iIsMount = __MountAtkSkill(GetMountVnum(), false);
		if (iIsMount!=MOUNT_TYPE_MILITARY)
		{
			if (test_server)
				sys_log(0, "CanUseSkill: Mount can't skill. vnum(%u) type(%d)", GetMountVnum(), iIsMount);
			return false;
		}
#endif
		switch(dwSkillVnum)
