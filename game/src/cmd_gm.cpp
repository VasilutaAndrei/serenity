...
ACMD(do_buy_marble)
{
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "Nu exista acest mob.");
		return;
	}
	
	if (!itemCount)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "Cantitatea este prea mare.");
		return;
	}

	if (itemCount <= 0)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "Cantitatea nu poate fi mai mica de 1.");
		return;
	}
	
	if (itemCount > 10)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "Cantitatea mai mare de 10.");
		return;
	}
	...
	*/
	switch(mobVnum)
	{
		...
		default:
		{
			ch->ChatPacket(CHAT_TYPE_INFO, "Nu exista acet glob de transformare.");
			price = 0;
			break;
		}
	}
	...
	if (price == 0)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "Pret gresit.");
		return;
	}
	
	if (ch->GetGold() < price*itemCount)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "Nu ai suficient yang.");
		return;
	}
	...
	for (int createMarble = 0; createMarble < itemCount; createMarble++)
	{
		...
		{
			ch->ChatPacket(CHAT_TYPE_INFO, "#%d nu exista acet vnum.", 70104);
			break;
		}
	}
}
...
struct FuncPurge
{
	...
	void operator () (LPENTITY ent)
	{
		...
		if (!m_bAll && iDist >= 1000)	// 10???I ??≫o?ˇ ?O´A °Iμe?? purge C?Ao ??´A´?.
			return;
		...
		if (pkChr->IsNPC() && !pkChr->IsPet() && !pkChr->IsNewPet() && !pkChr->IsMount() && !pkChr->IsSupport() && pkChr->GetRider() == NULL)
#else
...
ACMD(do_item_purge)
{
#ifdef ENABLE_ITEM_PURGE_RENEWAL
	char arg1[256];
	one_argument(argument, arg1, sizeof(arg1));
	...
	if (!*arg1)
	{
		for (i = 0; i < INVENTORY_MAX_NUM; ++i)
		{
			...
			{
				ITEM_MANAGER::instance().RemoveItem(item, "PURGE_INV");
				ch->SyncQuickslot(QUICKSLOT_TYPE_ITEM, i, 255);
#ifdef ENABLE_NEW_QUICK_SLOT_SYSTEM
				ch->SyncNewQuickslot(QUICKSLOT_TYPE_ITEM, i, QUICKSLOT_NEW_POS);
#endif
			}
		}
		return;
	...
	switch (LOWER(arg1[0]))
	{
		...
		case 'e':
			for (i = INVENTORY_MAX_NUM; i < BELT_INVENTORY_SLOT_START; ++i)
			{
				if ((item = ch->GetInventoryItem(i)))
				{
					ITEM_MANAGER::instance().RemoveItem(item, "PURGE_EQUIP");
					ch->SyncQuickslot(QUICKSLOT_TYPE_ITEM, i, 255);
				}
			}
			break;
		...
		case 'd':
			for (i = 0; i < DRAGON_SOUL_INVENTORY_MAX_NUM; ++i)
			{
				...
				{
					ITEM_MANAGER::instance().RemoveItem(item, "PURGE_DS");
				}
			}
			break;
	
		case 'b':
			for (i = BELT_INVENTORY_SLOT_START; i < BELT_INVENTORY_SLOT_END; ++i)
			{
				if ((item = ch->GetInventoryItem(i)))
				{
					ITEM_MANAGER::instance().RemoveItem(item, "PURGE_BELT");
					ch->SyncQuickslot(QUICKSLOT_TYPE_ITEM, i, 255);
				}
			}
			break;
		...
		case 'a':
			for (i = 0; i < INVENTORY_AND_EQUIP_SLOT_MAX; ++i)
			{
				if ((item = ch->GetInventoryItem(i)))
				{
					ITEM_MANAGER::instance().RemoveItem(item, "PURGE_ALL");
					ch->SyncQuickslot(QUICKSLOT_TYPE_ITEM, i, 255);
				}
...
ACMD(do_who)
{
	...
	ch->ChatPacket(CHAT_TYPE_INFO, "Total online %d / Shinsoo: %d / Chunjo: %d / Jinno: %d (this server %d)",
...
ACMD(do_user)
{
	...
	ch->ChatPacket(CHAT_TYPE_INFO, "Total jucatori online: %d", func.count);
}
...
ACMD(do_disconnect)
{
	...
	if (!*arg1)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "ex) /kick <player name>");
		return;
	}
...
typedef enum do_set_types_s {GOLD, RACE, SEX, JOB, EXP, MAX_HP, MAX_SP, SKILL, ALIGNMENT, ALIGN, BATTLE, GEM} do_set_types_t;
}
...
ACMD(do_set)
{
	...
	switch (i)
	{
		...
		case DoSetTypes::ALIGNMENT: // alignment
			...
			break;
#ifdef __ENABLE_BATTLE_FIELD__
		case DoSetTypes::BATTLE:
			{
				long battle = 0;
				str_to_number(battle, arg3);
				tch->PointChange(POINT_BATTLE_POINT, battle, true);
			}
			break;
#endif
...
{
	CBattleZone::instance().CloseEnter(true);
}
...
ACMD(do_battle_set_event)
{
	...
	CBattleZone::instance().SetEventInfo(month, day);
}
#endif
...
ACMD(do_attendance)
{
	...
	if(iCommand == 1)
	{
		if(quest::CQuestManager::instance().GetEventFlag("enable_attendance_event") == 0)
		{
			...
			SendNotice("Evenimentul Spintecatorul de Sefi este acum Activ, verifica iconita Event -");
			SendNotice("din partea dreapta a jocului pentru mai multe detalii.");
		}
		else
		{
			ch->ChatPacket(CHAT_TYPE_INFO, "Evennimentul este deja pornit.");
		}
	}
	else
	{
		quest::CQuestManager::instance().RequestSetEventFlag("enable_attendance_event", 0);
		ch->ChatPacket(CHAT_TYPE_INFO, "Ai dezactivat evenimentul.");
	}
}
...
ACMD(do_easter_event)
{
	...
	if(iCommand == 1)
	{
		if(quest::CQuestManager::instance().GetEventFlag("enable_easter_event") == 0)
		{
			...
			SendNotice("Evenimentul de paste este acum activ.");
		}
		else
		{
			ch->ChatPacket(CHAT_TYPE_INFO, "Evenimentul este deschis deja.");
		}
	}
	else
	{
		quest::CQuestManager::instance().RequestSetEventFlag("enable_easter_event", 0);
		ch->ChatPacket(CHAT_TYPE_INFO, "Ai oprit evenimentul de paste.");
	}
}
#endif
