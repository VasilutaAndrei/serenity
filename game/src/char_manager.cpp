...
#endif
{
	...
	if (ch->IsNPC() && !ch->IsPet() && !ch->IsNewPet() && !ch->IsMount() && ch->GetRider() == NULL)
#else	
...
LPCHARACTER CHARACTER_MANAGER::SpawnMobRandomPosition(DWORD dwVnum, long lMapIndex)
{
	...
	if (dwVnum >= 6915 && dwVnum <= 6919 && !quest::CQuestManager::instance().GetEventFlag("enable_attendance_event"))
	{
