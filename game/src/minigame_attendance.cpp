...
void CMiniGame::StartAttendanceRegen()
{
	const char * c_apszRegenNames[4] = {
		"quest/attendance/nemere_regen.txt",
		"quest/attendance/razador_regen.txt",
		"quest/attendance/red_dragon_regen.txt",
		"quest/attendance/beran_setaou_regen.txt"
	};
...
void CMiniGame::StartEasterStoneRegen()
{
	DWORD stoneMaps[20] = {
		...
		301, 302, 303, 304, 210 // indexurile mapelor beta 
	};
	...
	snprintf(szFileName10, sizeof(szFileName10), "%s/quest/attendance/easter/stone_level_10.txt", LocaleService_GetBasePath().c_str());
	snprintf(szFileName20, sizeof(szFileName20), "%s/quest/attendance/easter/stone_level_20.txt", LocaleService_GetBasePath().c_str());
	snprintf(szFileName30, sizeof(szFileName30), "%s/quest/attendance/easter/stone_level_30.txt", LocaleService_GetBasePath().c_str());
	snprintf(szFileName40, sizeof(szFileName40), "%s/quest/attendance/easter/stone_level_40.txt", LocaleService_GetBasePath().c_str());
	snprintf(szFileName50, sizeof(szFileName50), "%s/quest/attendance/easter/stone_level_50.txt", LocaleService_GetBasePath().c_str());
	snprintf(szFileName60, sizeof(szFileName60), "%s/quest/attendance/easter/stone_level_60.txt", LocaleService_GetBasePath().c_str());
	snprintf(szFileName70, sizeof(szFileName70), "%s/quest/attendance/easter/stone_level_70.txt", LocaleService_GetBasePath().c_str());
	snprintf(szFileName80, sizeof(szFileName80), "%s/quest/attendance/easter/stone_level_80.txt", LocaleService_GetBasePath().c_str());
	snprintf(szFileName90, sizeof(szFileName90), "%s/quest/attendance/easter/stone_level_90.txt", LocaleService_GetBasePath().c_str());
	snprintf(szFileName95, sizeof(szFileName95), "%s/quest/attendance/easter/stone_level_95.txt", LocaleService_GetBasePath().c_str());
	...
	for(int i = 0; i < sizeof(stoneMaps)/sizeof(stoneMaps[0]); i++)
	{
		if (map_allow_find(stoneMaps[i]))
		{
			if (LPSECTREE_MAP pkMap = SECTREE_MANAGER::instance().GetMap(stoneMaps[i]))
			{
				...
				else if(stoneMaps[i] == 301 || stoneMaps[i] == 302 || stoneMaps[i] == 303 || stoneMaps[i] == 304 || stoneMaps[i] == 210)
				{
...
{
	if(pkAttacker == NULL || pkVictim == NULL)
		return;
	
	if (!pkAttacker->GetDesc())
		return;
	
	if(pkAttacker->GetRaceNum() >= 6915 && pkAttacker->GetRaceNum() <= 6919)
	{
		if(pkVictim->IsPC())
		{
	...
	if(pkVictim->GetRaceNum() >= 6915 && pkVictim->GetRaceNum() <= 6919)
	{
		bool bContain = false;
		DWORD dwCount = 1;
...
void CMiniGame::AttendanceEventRequestReward(LPCHARACTER pChar)
{
	...
	{
		pChar->ChatPacket(CHAT_TYPE_INFO,("Acest eveniment este acum activat."));
		return;
	}
	
	if(pChar->GetQuestFlag(szClearName) == 0)
	{
		pChar->ChatPacket(CHAT_TYPE_INFO,("Trebuie sa castigi aceasta recompensa, inainte sa o primesti."));
		return;
	}
	
	if(pChar->GetQuestFlag(szRewardName))
	{
		pChar->ChatPacket(CHAT_TYPE_INFO,("Ai primit deja recompensa pe ziua de azi."));
		return;
	}
	
	if(pChar->GetLevel() < 30)
	{
		pChar->ChatPacket(CHAT_TYPE_INFO,("Ai nevoie de nivelul 30 pentru acest eveniment."));
		return;
	}
