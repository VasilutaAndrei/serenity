...
{
	static CGrid s_grid1(5, SPECIAL_INVENTORY_MAX_NUM/5 / 4);
	static CGrid s_grid2(5, SPECIAL_INVENTORY_MAX_NUM/5 / 4);
	static CGrid s_grid3(5, SPECIAL_INVENTORY_MAX_NUM/5 / 4);
	static CGrid s_grid4(5, SPECIAL_INVENTORY_MAX_NUM/5 / 4);
	...
	for (i = 0; i < SPECIAL_INVENTORY_MAX_NUM / 4; ++i)
	{
		if(bStorageType < 0 || bStorageType > 2)
			continue;
	...
	for (i = SPECIAL_INVENTORY_MAX_NUM / 4; i < SPECIAL_INVENTORY_MAX_NUM; ++i)
	{
		if(bStorageType < 0 || bStorageType > 2)
			continue;
		...
		s_grid2.Put(i - SPECIAL_INVENTORY_MAX_NUM / 4, 1, item->GetSize());
	}
	...
	for (i = 0; i < EXCHANGE_ITEM_MAX_NUM; ++i)
	{
		...
		if((bStorageType == 0 && (!item->IsUpgradeItem())) || (bStorageType == 1 && (!item->IsBook())) || (bStorageType == 2 && (!item->IsStone())))
			continue;
