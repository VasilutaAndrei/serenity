...
bool ITEM_MANAGER::Initialize(TItemTable * table, int size)
{
	...
	for (i = 0; i < size; ++i)
	{
		...
		if (m_vec_prototype[i].bType == ITEM_QUEST || IS_SET(m_vec_prototype[i].dwFlags, ITEM_FLAG_QUEST_USE | ITEM_FLAG_QUEST_USE_MULTIPLE)
			...
			|| (m_vec_prototype[i].bType == ITEM_NORMAL_PET )
#endif
		)
...
void ITEM_MANAGER::CreateQuestDropItem(LPCHARACTER pkChr, LPCHARACTER pkKiller, std::vector<LPITEM> & vec_item, int iDeltaPercent, int iRandRange)
{
	...
	// souldrop 
	if ((pkKiller->GetWear(ITEM_RING)) && GetDropPerKillPct(100, 2000, iDeltaPercent, "soul_drop") >= number(1, iRandRange))
	{
		const static DWORD soulgiver = 71051;

		pkKiller->AutoGiveItem (soulgiver, 1);
	}
	// souldrop 2
	if ((pkKiller->GetWear(ITEM_RING)) && GetDropPerKillPct(200, 2000, iDeltaPercent, "soul_drop2") >= number(1, iRandRange))
	{
		const static DWORD soulgiver2 = 71052;

		pkKiller->AutoGiveItem (soulgiver2, 1);
	}
