...
void CNewPetActor::ItemCubeFeed(int type)
{
	for (int i = 0; i < 9; ++i) {
		if (m_dwpetslotitem[i] != -1) {
			...
			if(type == 1)
			{
				if (itemxp->GetVnum() >= 55401 && itemxp->GetVnum() <= 55409 || itemxp->GetVnum() >= 55701 && itemxp->GetVnum() <= 55709 || itemxp->GetVnum() == 55001) {
			...
			else if(type == 3)
			{
				if (GetLevel() < gPlayerMaxLevel)
				{
					...
					{
						m_pkOwner->ChatPacket(CHAT_TYPE_INFO, "Pet-ul poate manca doar arme ,armuri si accesorii.");				
					}
				}
				else
				{
					m_pkOwner->ChatPacket(CHAT_TYPE_INFO, "Pet-ul tau are deja nivel maxim. Nu il mai poti hrani.");	
				}
			}
		}
	}
...
bool CNewPetActor::IncreasePetEvolution() {
	if (m_dwevolution < 3) {
		if (GetLevel() >= 40 && m_dwevolution < 1 || GetLevel() >= 80 && m_dwevolution < 2 || GetLevel() >= 81 && m_dwevolution < 3) {
			m_dwevolution += 1;
...
void CNewPetActor:: IncreasePetBonus() {
	...
	if (tmplevel % 5 == 0) {
		m_dwbonuspet[0][1] += number(3, 6);

	}
	if (tmplevel % 5 == 0) {
		m_dwbonuspet[1][1] += number(3, 6);
	}
	if (tmplevel % 5 == 0) {
		m_dwbonuspet[2][1] += number(3, 6);
	}
...
void CNewPetActor::SetExp(DWORD exp, int mode)
{
	...
	if(mode == 0){
		if(GetExp() >= GetNextExpFromMob())
		{
			if(GetEvolution() == 0 && GetLevel() == 40)
				...
				return;
			else if(GetEvolution() == 2 && GetLevel() == 81)
				return;
		}
	}else if(mode == 1)
	{
	...
	if (mode == 0) {
		if (GetExp() >= GetNextExpFromMob()) {
			if (GetExp() >= GetNextExpFromMob()){
				m_pkOwner->ChatPacket(CHAT_TYPE_INFO, "Insotitorul tau a ajuns la nivelul : %d ", GetLevel() +1);
				SetLevel(GetLevel() + 1);
				...
				m_dwexpitem = 0;
				m_pkChar->SetExp(0);
				m_pkOwner->ChatPacket(CHAT_TYPE_COMMAND, "PetExp %d %d %d", m_dwexp, m_dwexp, m_pkChar->PetGetNextExp());
				return;
			}else {
				m_pkOwner->ChatPacket(CHAT_TYPE_COMMAND, "PetExp %d %d %d", m_dwexp, m_dwexp, m_pkChar->PetGetNextExp());
			}
...
void CNewPetActor::Unsummon()
{
	...
	{
		DBManager::instance().DirectQuery("UPDATE new_petsystem SET level = %d, evolution=%d, exp=%d, expi=%d, bonus0=%d, bonus1=%d, bonus2=%d, skill0=%d, skill0lv= %d, skill1=%d, skill1lv= %d, skill2=%d, skill2lv= %d, duration=%d, tduration=%d WHERE id = %lu ", this->GetLevel(), this->m_dwevolution, this->GetExp(), this->GetExp(), this->m_dwbonuspet[0][1], this->m_dwbonuspet[1][1], this->m_dwbonuspet[2][1], this->m_dwskillslot[0], this->m_dwskill[0], this->m_dwskillslot[1], this->m_dwskill[1], this->m_dwskillslot[2], this->m_dwskill[2], this->m_dwduration, this->m_dwtduration, ITEM_MANAGER::instance().FindByVID(this->GetSummonItemVID())->GetID());
		this->ClearBuff();
		...
		ClearBuff();
		m_pkOwner->ChatPacket(CHAT_TYPE_INFO, "Insotitorul tau a fost alungat !");
...
DWORD CNewPetActor::Summon(const char* petName, LPITEM pSummonItem, bool bSpawnFar)
{
	...
	pSummonItem->SetForceAttribute(4, 1, m_dwtduration);
	pSummonItem->SetSocket(1,m_dwlevel);
	pSummonItem->SetSocket(2,m_dwevolution);
	pSummonItem->SetSocket(0,true);
	pSummonItem->Lock(true);
	m_pkOwner->ChatPacket(CHAT_TYPE_INFO, "L-ai chemat pe %s[Nivel:%d, Evolutie: %d] ", m_name.c_str(), m_dwlevel, m_dwevolution);
	return m_dwVID;
}
...
bool CNewPetActor::_UpdateFollowAI()
{
	...
	{
		float fOwnerRot = m_pkOwner->GetRotation() * 3.141592f / 160.f;
...
{
	int get_affect_pet[] = {AFFECT_NEW_PET2, AFFECT_NEW_PET3, AFFECT_NEW_PET4, AFFECT_NEW_PET5};
	int	DuratingBonus[] = {60*60*24*365};
	int pNew_PetBns1[] = {POINT_ATTBONUS_MONSTER, 10};	
	int pNew_PetBns2[] = {POINT_SKILL_DAMAGE_BONUS, 10};	
	int pNew_PetBns3[] = {POINT_ATTBONUS_HUMAN, 10};	
	int pNew_PetBns4[] = {POINT_SKILL_DEFEND_BONUS, 10};	
	int petlvl = GetLevel();	
	if (petlvl >= 50 || petlvl <= 80 && m_dwevolution == 1) {
		m_pkOwner->AddAffect(get_affect_pet[0], pNew_PetBns1[0], pNew_PetBns1[1], 0, DuratingBonus[0], 0, false);
	}
	
	if (petlvl >= 81 || petlvl <= 100  && m_dwevolution >= 2) {
		m_pkOwner->AddAffect(get_affect_pet[1], pNew_PetBns2[0], pNew_PetBns2[1], 0, DuratingBonus[0], 0, false);
	}
	
	if (petlvl >= 101 || petlvl <= 119  && m_dwevolution == 3) {
		m_pkOwner->AddAffect(get_affect_pet[2], pNew_PetBns3[0], pNew_PetBns3[1], 0, DuratingBonus[0], 0, false);
	}
	
	if (petlvl >= 120  && m_dwevolution == 3) {
		m_pkOwner->AddAffect(get_affect_pet[3], pNew_PetBns4[0], pNew_PetBns4[1], 0, DuratingBonus[0], 0, false);
	}
	...
	if (GetLevel() > 81 && m_dwevolution == 3) {
		for (int s = 0; s < 3; s++) {
			...
			case 1: //Rezistenta War 78		
			case 2: //Rezistenta Sura 80
			case 3: //Rezistenta Ninja 82	
			case 4: //Rezistenta Shaman 81	
			case 5: //Resistenza Nu cont
			case 6: //Valoare atac 53 Punti
			case 7: //Nu cont 97
			case 8: //Valoare magica %
			case 9: //Patrundere 16 
			case 11: //Abs PV
			case 12: //Abs MP
			case 13: // Blocare Coporala 27
			case 14: // Refexie 28
			case 15: // Drop Yang 44
			case 16: // ceva arc 172
...
{
	m_pkOwner->RemoveAffect(AFFECT_NEW_PET2);
	m_pkOwner->RemoveAffect(AFFECT_NEW_PET3);
	m_pkOwner->RemoveAffect(AFFECT_NEW_PET4);
	m_pkOwner->RemoveAffect(AFFECT_NEW_PET5);
