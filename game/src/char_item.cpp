...
// #ifdef __FAKE_BUFF__
	// if (pkOldItem && !pkOldItem->IsMoving() && CItemVnumHelper::IsFakeBuffSpawn(pkOldItem->GetVnum()))
...
#endif
	{
		...
		if (pkNewItem)
		{
			...
			{		
				...
				snprintf(szUpgradeAnnouncement, sizeof(szUpgradeAnnouncement), "<Informatie> [%s] a imbunatatit [%s] cu succes!", GetName(), pkNewItem->GetName()); 
...
bool CHARACTER::UseItemEx(LPITEM item, TItemPos DestCell)
{
	...
	// {
		...
		// if (item2->GetSocket(0) >= item2->GetValue(2))
		// {
			// ChatPacket(CHAT_TYPE_INFO,("Nu mai poti adauga suflete acestui obiect."));
			// ChatPacket(CHAT_TYPE_INFO,("Du-te la ma-ta si imbunatateste-l."));
		// }					
		// else
		// {
			// ChatPacket(CHAT_TYPE_INFO,("Ai acumulat %d din %d suflete!"),item2->GetSocket(0) + item->GetSocket(0), item2->GetValue(2));
	...
	if (item->GetVnum() == 91021)
	{
		...
		if (GetSupportSystem()->IsActiveSupport()) {
			...
			ChatPacket(CHAT_TYPE_INFO,("Nu poti face acest lucru daca shamanul este activ!"));
			return false;
		}					
					
		if (item2->GetAttributeValue(2) >= 1) {
			ChatPacket(CHAT_TYPE_INFO,("Ai adaugat deja un bonus int acestui saman."));
		}						
		else
		{
			ChatPacket(CHAT_TYPE_INFO,("Ai adaugat 35 de puncte int Samanului tau"));
			item2->SetForceAttribute(2, 0, 35);
			item->SetCount(item->GetCount() - 1);
		}
	}

	if (item->GetVnum() == 91022)
	{
		LPITEM item2;
		...
		{
			ChatPacket(CHAT_TYPE_INFO,("Ai adaugat 70 de puncte int Samanului tau"));
			item2->SetForceAttribute(2, 0, 70);
			item->SetCount(item->GetCount() - 1);
		}
	}

	if (item->GetVnum() == 91023)
	{
		LPITEM item2;
		...
		{
			ChatPacket(CHAT_TYPE_INFO,("Ai adaugat 100 de puncte int Samanului tau"));
			item2->SetForceAttribute(2, 0, 100);
			item->SetCount(item->GetCount() - 1);
		}
	}

	if (item->GetVnum() == 91024)
	{
		LPITEM item2;
		...
		if (item2->GetAttributeValue(2) <= 0) {
			ChatPacket(CHAT_TYPE_INFO,("Samanul tau nu are bonus int."));
		}						
		else
		{
			ChatPacket(CHAT_TYPE_INFO,("Bonusul de inteligenta aplicat samanului tau a fost sters!"));
			item2->SetForceAttribute(2, 0, 0);
			item->SetCount(item->GetCount() - 1);
		}
	}

	if (item->GetVnum() == 91025)
	{
		LPITEM item2;
		...
		if (item2->GetAttributeValue(3) + m_dwduration >= 28800) {
			ChatPacket(CHAT_TYPE_INFO,("Shamanul tau are o durabilitate mai mare de 20 de zile!"));
		}						
		else
		{
			item2->SetForceAttribute(3, 0, item2->GetAttributeValue(3) + m_dwduration);
			ChatPacket(CHAT_TYPE_INFO,("Durabilitatea Shamanului tau a fost extinsa cu succes!"));
			ChatPacket(CHAT_TYPE_INFO, "[%d ore]", m_dwduration /60);
			item->SetCount(item->GetCount() - 1);
		}	
	}

	if (item->GetVnum() == 91026)
	{
		LPITEM item2;
		int m_dwduration = number(2, 2) * 24 * 60;
	...
	if (item->GetVnum() == 91027)
	{
		LPITEM item2;
		int m_dwduration = number(7, 7) * 24 * 60;
	...
	if (item->GetVnum() == 91028)
	{
		LPITEM item2;
		int m_dwduration = number(14, 14) * 24 * 60;
	...
	if (item->GetVnum() == 91029)
	{
		...
		if (item2->GetSocket(2) >= 1 && item2->GetSocket(1) >= 1) {
			ChatPacket(CHAT_TYPE_INFO,("Frizura si costumul samanului tau au fost sterse!"));
			item2->SetSocket(1, 0);
			item2->SetSocket(2, 0);
			item->SetCount(item->GetCount() - 1);
		}						
		else if (item2->GetSocket(2) >= 1 && item2->GetSocket(1) == 0) {
			ChatPacket(CHAT_TYPE_INFO,("Frizura samanului tau a fost stersa!"));
			item2->SetSocket(2, 0);
			item->SetCount(item->GetCount() - 1);
		}						
		else if (item2->GetSocket(2) == 0 && item2->GetSocket(1) >= 1) {
			ChatPacket(CHAT_TYPE_INFO,("Costumul samanului tau a fost stersa!"));
			item2->SetSocket(1, 0);
			item->SetCount(item->GetCount() - 1);
		}						
		else if (item2->GetSocket(2) == 0 && item2->GetSocket(1) == 0) {
			ChatPacket(CHAT_TYPE_INFO,("Shamanul tau nu are echipat nici un costum/frizura!"));
		}						
	}
	...
	if (item->GetVnum() == 55001) {
		...
		if (item2->GetVnum() > 55709 || item2->GetVnum() < 55701)
			return false;
		...
		if (pmsg2->Get()->uiNumRows > 0) {
			...
			if (atoi(row[0]) > 0) {
				if (GetNewPetSystem()->IsActivePet()) {
					ChatPacket(CHAT_TYPE_INFO, "Nu poti face acest lucru cat timp companiotul este activ.");
					return false;
				}

				DBManager::instance().DirectQuery("UPDATE new_petsystem SET duration =(tduration) WHERE id = %d", item2->GetID());
				ChatPacket(CHAT_TYPE_INFO, "Durata de viata a insotitorului tau este plina.");
			}
			else {
				DBManager::instance().DirectQuery("UPDATE new_petsystem SET duration =(tduration/2) WHERE id = %d", item2->GetID());
				ChatPacket(CHAT_TYPE_INFO, "Durata de viata a insotitorului tau a fost restaurata.");
			}
			item->SetCount(item->GetCount() - 1);
			return true;
		}
	...
	if (item->GetVnum() >= 55701 && item->GetVnum() <= 55708) {
		LPITEM item2;
	...
	if (item->GetVnum() == 55002 && item->GetAttributeValue(0) > 0) {
		...
		if(pos == -1)
		{
			ChatPacket(CHAT_TYPE_INFO, "Nu ai spatiu suficient.");
			return false;
		}
...
#ifdef __ENABLE_BATTLE_FIELD__
							if (item->GetVnum() == 27225)
							{
								if (FindAffect(AFFECT_BATTLE_POTION))
								{
									ChatPacket(CHAT_TYPE_INFO,("Ai folosit deja aceasta licoare."));
									return false;
								}
						
								if (CBattleZone::instance().IsBattleZoneMapIndex(GetMapIndex()) == true)
								{
									int iMultiply = 0;
									if(GetLevel() <= 64)
									{
								...
								{
									ChatPacket(CHAT_TYPE_INFO,("Licoarea de lupta poate fi folosita doar in Zona de Razboi."));
									return false;
								}
							}
#endif						
						...
						for (int i = 0; i < count; i++){
							switch (dwVnums[i])
							{
								case CSpecialItemGroup::GOLD:
									ChatPacket(CHAT_TYPE_INFO, ("O monede se zareste pe fundul acestei cutii!"));
									ChatPacket(CHAT_TYPE_INFO, ("Ai primit %d Yang."), dwCounts[i]);
									break;
				...
				switch (item->GetSubType())
				{
					...
					case USE_SPECIAL:
						...
						switch (item->GetVnum())
						{
							...
							case REWARD_BOX_UNIQUE_ITEM_CAPE_OF_COURAGE_SECOND:
								AggregateMonster();
								break;
								
							case REWARD_BOX_UNIQUE_ITEM_CAPE_OF_COURAGE_THIRD:
								AggregateMonster();
								break;
								
							case REWARD_BOX_UNIQUE_ITEM_CAPE_OF_COURAGE_FORTH:
								AggregateMonster();
								break;
...
#ifdef __FAKE_BUFF__
								if (item->GetVnum() >= 91010 && item->GetVnum() <= 91021)
								{
									...
									// {
										...
										// else
										// {
											// ChatPacket(CHAT_TYPE_INFO, ("Ai chemat deja un Buffi."));
											// return false;
										// }
									// }
									// else
									// {
										// if (GetLevel() < 5)
										// {
											// ChatPacket(CHAT_TYPE_INFO, ("Ai nevoie de nivelul 5 pentru a chema un buff."));
											// return false;
										// }

										// bool bSuccess = false;
										// for (int i = 0; i < 25 && !bSuccess; ++i)
											// bSuccess = FakeBuff_Owner_Spawn(GetX() + number(-200, 200), GetY() + number(-200, 200), item);

										// if (!bSuccess)
											// ChatPacket(CHAT_TYPE_INFO, ("Nu poti chema un buff aici."));
										// else
											// ChatPacket(CHAT_TYPE_INFO, ("Buffi a fost violat de blody."));
									// }
									FakeBuff_Owner_Spawn(GetX() + number(-200, 200), GetY() + number(-200, 200), item);
									ChatPacket(CHAT_TYPE_INFO, ("Test buffi."));
								}
#endif
					...
					case USE_COSTUME_TRANSFORM:
						{
							...
							{
								ChatPacket(CHAT_TYPE_INFO,("<Sistem>Acest pergament poate fi folosit doar pe costume."));
								return false;
							}
								...
								case USE_COSTUME_ENCHANT:
									{
										...
										item2->ChangeAttribute(aiChangeProb);
										ChatPacket(CHAT_TYPE_INFO,("<Sistem>Ai schimbat bonusul cu succes!"));
										{
											char buf[21];
											snprintf(buf, sizeof(buf), "%u", item2->GetID());
								...
								case USE_COSTUME_TRANSFORM:
									{
										...
										int aiChangeProb[ITEM_ATTRIBUTE_MAX_LEVEL] = {100, 10, 5, 0, 0};
										item2->ChangeAttribute(aiChangeProb);
										ChatPacket(CHAT_TYPE_INFO,("<Sistem>Bonusurile de pe costumul tau au fost transformte cu succes!"));
										{
											char buf[21];
											snprintf(buf, sizeof(buf), "%u", item2->GetID());
...
#ifdef ENABLE_CHANGELOOK_EXTEND
	if (!CanBeWearByAntiflag(item->GetTransmutation()))
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("YOU_CAN_NOT_USE_THIS_BECAUSE_OF_CHANGE_LOOK_ITEM_ANTIFLAG"));
		return false;
	}
	
	if (!CanBeWearBySex(item->GetTransmutation()))
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("YOU_CAN_NOT_USE_THIS_BECAUSE_OF_CHANGE_LOOK_ITEM_SEX"));
		return false;
	}
#endif	
...
#ifdef ITEM_REMAINING
//fish_chest_open
	if (item->GetVnum() == EXCELENT_BOSS1)
	{
		if (GetExchange() || GetMyShop() || GetShopOwner() || IsOpenSafebox() || IsCubeOpen())
		{
			ChatPacket(CHAT_TYPE_INFO, "Nu poti folosi cufarul cand ai deschisa o noua fereastra!");
			return false;
		}
		if (true == IsDead())
		{
			ChatPacket(CHAT_TYPE_INFO, "Nu poti deschide cufarul cand esti mort!");
			return false;
		}	
		else
	{			
		{
			item->SetSocket(0, item->GetSocket(0) + 1);			
			if (item->GetSocket(0) >= EXCELENT_USINGS1)
			{
			ITEM_MANAGER::instance().RemoveItem(item);
			}		
		}			
	...
	if (item->GetVnum() == EXCELENT_BOSS2)
	{
		if (GetExchange() || GetMyShop() || GetShopOwner() || IsOpenSafebox() || IsCubeOpen())
		{
			ChatPacket(CHAT_TYPE_INFO, "Nu poti folosi cufarul cand ai deschisa o noua fereastra!");
	...
	{			
		{
			item->SetSocket(0, item->GetSocket(0) + 1);			
			if (item->GetSocket(0) >= EXCELENT_USINGS2)
			{
			ITEM_MANAGER::instance().RemoveItem(item);
			}		
		}			
	...
	if (item->GetVnum() == EXCELENT_BOSS3)
	{
		if (GetExchange() || GetMyShop() || GetShopOwner() || IsOpenSafebox() || IsCubeOpen())
		{
			ChatPacket(CHAT_TYPE_INFO, "Nu poti folosi cufarul cand ai deschisa o noua fereastra!");
	...
	{			
		{
			item->SetSocket(0, item->GetSocket(0) + 1);			
			if (item->GetSocket(0) >= EXCELENT_USINGS3)
			{
			ITEM_MANAGER::instance().RemoveItem(item);
			}		
		}			
	...
	if (item->GetVnum() == EXCELENT_BOSS4)
	{
		if (GetExchange() || GetMyShop() || GetShopOwner() || IsOpenSafebox() || IsCubeOpen())
		{
			ChatPacket(CHAT_TYPE_INFO, "Nu poti folosi cufarul cand ai deschisa o noua fereastra!");
	...
	{			
		{
			item->SetSocket(0, item->GetSocket(0) + 1);			
			if (item->GetSocket(0) >= EXCELENT_USINGS4)
			{
			ITEM_MANAGER::instance().RemoveItem(item);
			}		
		}			
...
bool CHARACTER::DestroyItem(TItemPos Cell)
{
    ...
    ITEM_MANAGER::instance().RemoveItem(item);
	ChatPacket(CHAT_TYPE_INFO,("<Sterge Item> Ai sters : |cFFc9ff00|H|h[%s]."), item->GetName());
    return true;
}
	...
	else
	{
		...
		else if (item->IsStone())
		{
			if (DestCell.window_type != STONE_INVENTORY)
			{
				...
				UPGRADE_INVENTORY == DestCell.window_type ||
				BOOK_INVENTORY == DestCell.window_type ||
				STONE_INVENTORY == DestCell.window_type)
			return false;
#else
		else if (DRAGON_SOUL_INVENTORY == DestCell.window_type)
			return false;
...
#ifdef ENABLE_PET_SLOT
			if(ITEM_NORMAL_PET == item->GetType() || ITEM_NORMAL_BUFF == item->GetType() || ITEM_NORMAL_TITLE == item->GetType())
			{
				quest::CQuestManager::instance().UseItem(GetPlayerID(), item, false);
			}
#endif				
					...
					if (item->IsUpgradeItem())
					{
						...
						for (int i = 0; i < SPECIAL_INVENTORY_MAX_NUM; ++i)
						{
							...
							if (item2->GetVnum() == item->GetVnum())
							{
								...
								ChatPacket(CHAT_TYPE_INFO,("%s a fost mutat automat in depozitul special"), item2->GetName());
						...
						ChatPacket(CHAT_TYPE_INFO,("%s a fost mutat automat in depozitul special"), item->GetName());
					}
					else if (item->IsBook())
					{
						BYTE bCount = item->GetCount();
...
#ifdef ENABLE_PET_SLOT
		if(ITEM_NORMAL_PET == item2->GetType() || ITEM_NORMAL_BUFF == item2->GetType())
		{
...
bool CHARACTER::EquipItem(LPITEM item, int iCandidateCell)
{
	...
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<Horse> Nu poti schimba armura cand calaresti."));
		return false;
	}
	
	if (item->GetType() == ITEM_COSTUME && item->GetSubType() == COSTUME_BODY && IsRiding())
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<Horse> Nu poti schimba costumul cand calaresti."));
		return false;
	}
...
#ifdef ENABLE_PET_SLOT
		else if(ITEM_NORMAL_PET == item->GetType() || ITEM_NORMAL_BUFF == item->GetType() || ITEM_NORMAL_TITLE == item->GetType())
		{
			quest::CQuestManager::instance().UseItem(GetPlayerID(), item, false);
		}
#endif
...
bool  CHARACTER::InformationSpecialItemGroup(DWORD dwGroupNum, std::vector<DWORD> &dwItemVnums, 
											std::vector<DWORD> &dwItemCounts, std::vector <LPITEM> &item_gets, int &count)
{
	...
	bool bSuccess;
	DWORD CountTotal = pGroup->GetItemGroup();

	for (int i = 0; i < CountTotal; i++)
	{
		DWORD dwVnum = pGroup->GetVnum(i);
		DWORD dwCount = pGroup->GetCount(i);
		bSuccess = true;
...
bool CHARACTER::CanEquipNow(const LPITEM item, const TItemPos& srcCell, const TItemPos& destCell) /*const*/
{
	...
	if(item->GetType() == ITEM_NORMAL_BUFF) {
		
		if (GetMapIndex() == 113 || GetMapIndex() == 1 || GetMapIndex() == 21 || GetMapIndex() == 41) {
			ChatPacket(CHAT_TYPE_INFO,("Nu poti folosi shamanul in aceasta mapa!"));
			return false;			
		}
		
		if(item->GetAttributeValue(3) == 0) {
			ChatPacket(CHAT_TYPE_INFO,("Nu poti folosii acest item, deoarece durabilitatea a expirat"));
			return false;
		}
   ...
   if (item->GetType() == ITEM_COSTUME && item->GetSubType() == USE_RING) // ring check for two same rings
   {
      LPITEM ringItems[2] = { GetWear(WEAR_COSTUME_RING1), GetWear(WEAR_COSTUME_RING2) };
      for (int i = 0; i < 2; i++)
      {
...
bool CHARACTER::SwapItemToItem(TItemPos srcCell, TItemPos destCell)
{
	...
	if (item1->GetVnum() == 71051 || 71052 || 91021)
		return false;

	if (item1->GetVnum() > 91020 || item1->GetVnum() < 91030)	
		return false;
	
	if (item2->GetVnum() == 71051 || 71052 || 70032)
		return false;	
	
	if (item2->GetVnum() > 91020 || item2->GetVnum() < 91030)	
		return false;	
	...
	{
		BYTE bCell1 = item1->GetCell();
		BYTE pPageDest = bDestCell / (INVENTORY_MAX_NUM / 4); //1
		BYTE pPageDest2 = (((item1->GetSize()-1) * 5) + bDestCell) / (INVENTORY_MAX_NUM / 4); //10 + 40   / 45 = 1
		//BYTE pPageDest2 = (bDestCell + (5 * (item1->GetSize() - item2->GetSize()))) / (INVENTORY_MAX_NUM / 4);
		...
		if (pPageDest != pPageDest2)
		{
			...
			{
				pPageDest2 = (bDestCell + (5*(item1->GetSize()-1)) - (5 * ii)) / (INVENTORY_MAX_NUM / 4); //35 + 10 - 5
				if (pPageDest == pPageDest2)
				{
		...
		{
			BYTE bBusyCell = bCell + (5 * i);
			TItemPos busyCell(INVENTORY, bBusyCell);
			LPITEM busy = GetItem(busyCell);
			if (busy)
			{
				TItemPos newCell(INVENTORY, bCell1 + (5 * i));
				if (!IsEmptyItemGrid(newCell, busy->GetSize()))
				return false;
			}
		}
		item1->RemoveFromCharacter();
		for (int i = 0; i < item1->GetSize(); ++i)
		{
			...
			BYTE lpage = bBusyCell / (INVENTORY_MAX_NUM / 4);
			if (lpage != pPageDest)
				continue;
			TItemPos busyCell(INVENTORY, bBusyCell);
			LPITEM busy = GetItem(busyCell);
