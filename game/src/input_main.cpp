...
{
   return 0;//2012.05.17 ±e¿e¿i
   //0 : A¤≫oAuA¸·I ≫c¿e
   //1 : ±Y°­°æ ºIA·
   //2 : ±Y°­°æAI AOA¸³ª, °³AI≫oA¡¿¡¼­ ≫c¿eAß
   //3 : ±³E?Aß
   //4 : ¿¡·?
   int hyperlinks;
   bool colored;
...
int CInputMain::Whisper(LPCHARACTER ch, const char * data, size_t uiBytes)
{
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("BATTLE_ZONE_CANT_WHISPER"));
		return (iExtraLen);
	}
#endif	
...
void CInputMain::BraveRequestPetName(LPCHARACTER ch, const char* c_pData)
{
	...
	if (ch->CountSpecifyItem(vid) > 0 && check_name(p->petname) != 0) {
		...
		int tmpdur = number(14, 14) * 24 * 60;
		char szQuery1[1024];
		snprintf(szQuery1, sizeof(szQuery1), "INSERT INTO new_petsystem VALUES(%lu,'%s', 1, 0, 0, 0, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d)", item->GetID(), p->petname, number(23, 23), number(23, 23), number(23, 23), tmpskill[0], 0, tmpskill[1], 0, tmpskill[2], 0, tmpdur, tmpdur);
...
int CInputMain::Chat(LPCHARACTER ch, const char * data, size_t uiBytes)
{
	...
	if (CBattleZone::instance().IsBattleZoneMapIndex(ch->GetMapIndex()))
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("BATTLE_ZONE_CANT_CHAT"));
		return iExtraLen;
	}	
#endif
	...
	switch (pinfo->sub_header)
	{
		...
		case EXCHANGE_SUBHEADER_CG_ELK_ADD:	// arg1 == amount of gold
			if (ch->GetExchange())
			{
				...
				{
					ch->ChatPacket(CHAT_TYPE_INFO,("Limita de yang a fost atinsa!"));
...
bool CheckIsBlocked(LPCHARACTER ch, const char* c_szName)
{
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, ("%s nu poate fi gasit"), c_szName);
		return false;
	}
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, ("%s este deja banat"), c_szName);	
		return true;
	}
...
void CInputMain::SendAdminBanManager(LPCHARACTER ch, const char* c_pData)
{
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, ("Nu te poti bana singur"));	
		return;
	}
	...
	switch (p->action)
	{
		...
		case ACTION_BAN_TIME:
		{
			...
			if (p->action == ACTION_BAN_PERMANENTLY)
			{
				...
				ch->ChatPacket(CHAT_TYPE_INFO, ("%s a fost banat permanent cu succes"), szEscapedUserName);
			}
			else
			{
				if (p->duration < 300)
				{
					ch->ChatPacket(CHAT_TYPE_INFO, ("Timpul este invalid"));
					return;	
				}
				...
				ch->ChatPacket(CHAT_TYPE_INFO, ("%s a fost banat cu succes"), szEscapedUserName);
			}
		}
		break;
		...
		case ACTION_BAN_IP:
		{
			...
			ch->ChatPacket(CHAT_TYPE_INFO, ("Toate conturile cu ip-ul respectiv au fost banate permanent cu succes!"));
		}
		break;
		default:
			return;
...
void CInputMain::PartyInvite(LPCHARACTER ch, const char * c_pData)
{
	...
	if (CBattleZone::instance().IsBattleZoneMapIndex(ch->GetMapIndex()))
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("BATTLE_ZONE_CANT_INVITE_PARTY"));
		return;
	}
#endif
		...
		case GUILD_SUBHEADER_CG_ADD_MEMBER:
			{
				...
				if (!newmember->IsPC())
				{
					ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("THE ACTION HAS BEEN STOPED - EXPLOIT DETECTED"));
					return SubPacketLen;
				}
...
int CInputMain::Analyze(LPDESC d, BYTE bHeader, const char * c_pData)
{
	...
	switch (bHeader)
	{
		...
		case HEADER_CG_REQUEST_ENTER_BATTLE:
			CBattleZone::instance().RequestEnter(ch);
			break;
			
		case HEADER_CG_REQUEST_EXIT_BATTLE:
			CBattleZone::instance().RequestExit(ch);
			break;
#endif				
#ifdef __SEND_TARGET_INFO__
