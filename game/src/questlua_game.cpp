...
namespace quest
{
...
#ifdef __ENABLE_BATTLE_FIELD__
	int combat_zone_get_open_time_list(lua_State* L)
	{
		CBattleZone::instance().GetOpenTimeList(L);
		return 1;
	}
#endif	
	...
	int game_open_upgrade_items_storage(lua_State* L)
	{
		CQuestManager& q = CQuestManager::instance();
		LPCHARACTER ch = q.GetCurrentCharacterPtr();
		...
		p.bHeader = HEADER_GC_UPGRADE_ITEMS_STORAGE_OPEN;
		p.bSize = CUpgradeItemsStorage::UPGRADE_ITEMS_STORAGE_TAB_COUNT;

		ch->GetDesc()->Packet(&p, sizeof(TPacketCGSafeboxSize));
		ch->GetStorage()->SendItems(ch);
...
#ifdef __ENABLE_BATTLE_FIELD__
			{ "combat_zone_get_open_time_list",		combat_zone_get_open_time_list			},
#endif
