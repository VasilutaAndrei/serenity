...
void CInputDB::PlayerCreateSuccess(LPDESC d, const char * data)
{
	...
	if (china_event_server)
	{
		...
		static SInitialItem initialItems[JOB_MAX_NUM][MAX_INITIAL_ITEM] =
		{
			...
			{ {21023,	2}, {12223,	3}, {21513,	4}, { 6023,	1}, {16143,	8}, {17103,	9}, { 0,	0}, {13193,	11}, {14103, 12}, }, // NOTE: ������ �ʱ������.. �Ⱦ��̴� �ڵ�� �н�
#endif
		};
...
EVENTFUNC(fix_shop_event)
{
	...
	if (pRes->uiNumRows>0)
	{
		...
		{
			DWORD id,pid,vnum,count,socket[6],attr[7][2],look;
			int col = 0;
			...
			sprintf(query, "INSERT INTO player_gift SET owner_id=%d,vnum=%d,count=%d,look=%d,reason='%s',`from`=replace(\"%s\",' ','_'),status='WAIT',date_add=NOW()",
				pid, vnum,count,look, LC_TEXT("SHOP_REASON"), row[3]);
...
{
	BYTE bResult = *(const BYTE*) c_pData;
