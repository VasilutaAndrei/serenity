...
enum EPointTypes
{
	...
	POINT_BATTLE_POINT 		= 148,
	POINT_TEMP_BATTLE_POINT = 149,
#endif
...
#ifdef __UPGRADE_ITEMS_STORAGE__
		DWORD			GetID() const	{ return m_dwPlayerID; }
#endif
...
//		bool			IsPet() const		{ return m_bCharType == CHAR_TYPE_PET; }
	...
	public:
		...
		bool			InformationSpecialItemGroup(DWORD dwGroupNum, std::vector <DWORD> &dwItemVnums, 
							std::vector <DWORD> &dwItemCounts, std::vector <LPITEM> &item_gets, int &count);							
	...
	public:
		...
		bool			CreateGaya(int glimmerstone_count, LPITEM metinstone_item, long long cost, int pct);
#endif
...
//		AttackLog			m_kAttackLog;
	...
	public:
		...
		void 				BuffSummon(LPITEM buffItem);
		void 				BuffUnsummon(LPITEM buffItem);
		void 				CheckBuff();
		bool 				IsBuffSummon();
#endif
	protected:
...
#endif
		LPEVENT				m_pksupportSystemExpireEvent;
...
#ifdef __ENABLE_BATTLE_FIELD__
	public:
		long			GetBattlePoints() const		{ return m_points.battlePoint;	}
		void			SetBattlePoints(long battlePoint)	{ m_points.battlePoint = battlePoint;	}
		
		BYTE			GetBattleRank() const		{ return m_bBattleRank;	}
		void			SetBattleRank(BYTE bRank)	{ m_bBattleRank = bRank; }
		
		BYTE			GetBattleDeadCount() const		{ return m_bBattleDeadCount;	}
		void			IncreaseBattleDeadCount()	{ m_bBattleDeadCount++; }
	protected:
		BYTE	m_bBattleRank;
		BYTE	m_bBattleDeadCount;
#endif
#ifdef __UPGRADE_ITEMS_STORAGE__
	private:
		CUpgradeItemsStorage* m_pkStorage;

	public:
		CUpgradeItemsStorage* GetStorage()  { return m_pkStorage; };
		void OpenUpgradeItemsStorage(LPCHARACTER pkOpener);
#endif
#ifdef OFFLINE_SHOP
	public:
	...
	public:
		...
		bool 	CanBeWearByAntiflag(DWORD dwVnum);
		bool 	CanBeWearBySex(DWORD dwVnum);
#endif
