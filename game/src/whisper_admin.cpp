...
void CWhisperAdmin::SaveLog(LPCHARACTER ch, const char* c_pszText, const char* c_pszLang, int color)
{
	...
	snprintf(szQuery, sizeof(szQuery), "INSERT INTO rb_log.whisper_system_message (who, date, text, lang, color) VALUES('%s', NOW(), '%s', '%s', '%d')", ch->GetName(), c_pszText, c_pszLang, color);
...
void CWhisperAdmin::Manager(LPCHARACTER ch, const char* c_pData)
{
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "[SYSTEM] Acest system poate fi folosit doar de admini.");
		return;
	}
