...
{
	#define TIME_FOR_WAIT_TO_USE_AGAIN	0*60*60 // You can use one item once every 6 hours, that means 4 items per day.
	...
	if (curTime && get_global_time() < curTime)
	{
		...
		ch->ChatPacket(CHAT_TYPE_INFO, "<Biolog> Trebuie sa astepti [%d H] [%d M] [%d S] pentru a restarta din nou timpul la biolog.", mHours, mMin, mSec);
		return;
	}
	...
	ch->ChatPacket(CHAT_TYPE_INFO, "<Biolog> Timpul a fost inlaturat cu succes, acum poti trimite un nou obiect pentru analizare.");
	ch->ChatPacket(CHAT_TYPE_COMMAND, "BINARY_Biolog_Update %d %d %d %d", pTableString[1][0] - init_biologTime(), pTableString[0][0], pTableCountMax[0], pTableVnum[0]);instance().SendUpdate_Binary(ch);
}
/*********************************************************************
* Initialization
*/
...
void SendMessage(LPCHARACTER ch, const char* pType)
{
	...
	if (!strcmp(pType, "succes"))
		szText = pTable_Translate[0];
	
	if (!strcmp(pType, "failed"))
		szText = pTable_Translate[1];
			
	if (!strcmp(pType, "completed"))
...
bool SendBiolog_Data(LPCHARACTER ch, int argument)
{
	...
	{	
		...
		if (iRandom <= iPass_percent)
		{
			...
			{
				ch->ChatPacket(CHAT_TYPE_INFO, "[Biolog] Felicitari itemul a fost luat cu succes!");
				...
				if (argument <= 7)
				{
					ch->ChatPacket(CHAT_TYPE_INFO, "[Biolog] Felicitari, ai terminat aceasta misiune!"); 
				}
			...
			ch->ChatPacket(CHAT_TYPE_INFO, "[Biolog] Scuze, acest item a fost putred!"); 
			ch->RemoveSpecifyItem(pTableVnum[argument], 1);
			return false;
	}
	else
