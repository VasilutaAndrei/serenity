...
#ifdef __UPGRADE_ITEMS_STORAGE__
	m_pkStorage = new CUpgradeItemsStorage(this);
#endif
	Initialize();
}
...
void CHARACTER::OpenMyShop(const char * c_pszSign, TShopItemTable * pTable, BYTE bItemCount)
{
	...
	if (GOLD_MAX <= nTotalMoney)
	{
		...
		ChatPacket(CHAT_TYPE_INFO,("Limita de yang a fost atinsa!"));
		return;
	}
	...
	for (BYTE i = 0; i < bItemCount; ++i)
	{
		...
		if (pkItem)
		{
			...
			{
				ChatPacket(CHAT_TYPE_INFO, "Nu poti pune la vanzare un obiect transmutat.");
				return;
			}
...
#ifdef __ENABLE_BATTLE_FIELD__
		addPacket.bBattleRank = GetBattleRank();
#endif
...
#ifdef __ENABLE_BATTLE_FIELD__
	bool isBattle = CBattleZone::instance().IsBattleZoneMapIndex(GetMapIndex());
	
	pack.sAlignment	= isBattle ? 0 : m_iAlignment / 10;
	pack.bBattleRank = IsPC() && !isBattle ? GetBattleRank() : 0;
#else
...
#endif
#ifdef __ENABLE_BATTLE_FIELD__
	tab.battlePoint = GetBattlePoints();
#endif
...
#endif
#ifdef __ENABLE_BATTLE_FIELD__
	pack.points[POINT_BATTLE_POINT] = GetBattlePoints();
...
#ifdef ENABLE_FIX_TINCTURA_WITHOUT_RELOG
	UpdatePacket();
	SET_BIT(m_bAddChrState, ADD_CHARACTER_STATE_SPAWN);
	m_afAffectFlag.Set(AFF_SPAWN);
	ViewReencode();
...
#endif
#ifdef __ENABLE_BATTLE_FIELD__
	SetBattlePoints(t->battlePoint);
#endif

	SetMapIndex(t->lMapIndex);
	SetXYZ(t->x, t->y, t->z);
// #ifdef __TITLE_SYSTEM__
	// SetPoint(POINT_TITLE, t->iTChoosed);
	// int w = 1;
	// const std::string text = t->cTAvailable;
	// std::size_t start = 0, end = 0;
	// while ((end = text.find(',', start)) != std::string::npos)
	// {
		// BYTE bType = POINT_TITLE + w;
		// std::string sString = text.substr(start, end - start);
		// SetPoint(bType, atoi(sString.c_str()));
		// w += 1;
		// start = end + 1;
	// }
// #endif
...
#endif
#ifdef __FAKE_BUFF__
	m_stFakeBuffName = t->fakebuff_name;
#endif
}
...
void CHARACTER::PointChange(BYTE type, long long amount, bool bAmount, bool bBroadcast)
{
	...
	switch (type)
	{
		...
		case POINT_LEVEL:
			...
			switch (val)
			{
				case 75:	case 90:	case 105:	case 115:	case 120:
					char szNoticeLevelUp[QUERY_MAX_LEN];
					snprintf(szNoticeLevelUp, sizeof(szNoticeLevelUp), "<Informatie> [CH%d]: [%s] a atins nivelul [%u]. Felicitari!", g_bChannel, GetName(), GetLevel()); 
		...
		case POINT_BATTLE_POINT:
			{
				const int64_t nTotalBattlePoints = static_cast<int64_t>(GetBattlePoints()) + static_cast<int64_t>(amount);
				...
				SetBattlePoints(GetBattlePoints() + amount);
				val = GetBattlePoints();
			}
			break;
...
#endif
		{
			// 2005.03.17.myevan.?¸°??? ??´? °???´A °ł?I ≫oAˇ ?ł¸® ±a´E?≫ ??μ?˝??˛´?.
			if (GetMyShop())
			{
				if (pkChrCauser->IsDead() == true) return;

				//PREVENT_TRADE_WINDOW
				if (pkChrCauser == this) // ?U±a´A °ˇ´E
				{
					if ((GetExchange() || IsOpenSafebox() || GetShopOwner()) || IsCubeOpen())
					{
						pkChrCauser->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("´?¸? °?·ˇAß(?˘°i,±ł??,≫oAˇ)?ˇ´A °ł?I≫oAˇ?≫ ≫c?eC? ?o ??˝?´?´?."));
						return;
					}
				}
				else // ´?¸? ≫c¶÷?? ?￢¸?Cß?≫¶§
				{
					// ?￢¸?C? ≫c¶÷?? ±ł??/?˘°i/°ł?I≫oAˇ/≫oAˇ???eAß??¶o¸e ??°ˇ
					...
					// ?￢¸?C? ´e≫o?? ±ł??/?˘°i/≫oAˇ???eAß??¶o¸e ??°ˇ
					//if ((GetExchange() || IsOpenSafebox() || GetShopOwner()))
					if ((GetExchange() || IsOpenSafebox() || IsCubeOpen()))
					{
						pkChrCauser->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("≫o´e???? ´?¸? °?·ˇ¸| C?°i ?O´A Aß?O´?´?."));
						return;
					}
				}
		...
		m_bDropMetinStonePct = info.iDropPct;
		{
			...
			for (int iStoneLevel = 0; iStoneLevel < STONE_LEVEL_MAX_NUM; iStoneLevel ++)
			{
				...
				else
				{
					...
					m_dwDropMetinStone += 0; // 돌 +a -> +(a+1)이 될때마다 100씩 증가
				}
			}
		}
	}
	...
	for (int i = 0; i < INVENTORY_MAX_NUM; ++i)
	{
		...
		if (item->GetVnum())
		{
			switch (item->GetVnum())
			{
				...
				case 3210: case 3211: case 3212: case 3213: case 3214: case 3215: case 3216: case 3217: case 3218: case 3219:
				case 3160: case 3161: case 3162: case 3163: case 3164: case 3165: case 3166: case 3167: case 3168: case 3169:
				...
				case 7160: case 7161: case 7162: case 7163: case 7164: case 7165: case 7166: case 7167: case 7168: case 7169:
				case 5110: case 5111: case 5112: case 5113: case 5114: case 5115: case 5116: case 5117: case 5118: case 5119:
				case 5120: case 5121: case 5122: case 5123: case 5124: case 5125: case 5126: case 5127: case 5128: case 5129:
					continue;
			}
		}
...
bool CHARACTER::CanBeWearBySex(DWORD dwVnum)
{
	if(dwVnum == 0)
		return true;
	
	const TItemTable* itemTable = ITEM_MANAGER::instance().GetTable(dwVnum);
...
bool CHARACTER::CanBeWearByAntiflag(DWORD dwVnum)
{
	if(dwVnum == 0)
		return true;
	...
	{
		case JOB_WARRIOR:
			if (itemTable->dwAntiFlags & ITEM_ANTIFLAG_WARRIOR)
				return false;
			break;

		case JOB_ASSASSIN:
			if (itemTable->dwAntiFlags & ITEM_ANTIFLAG_ASSASSIN)
				return false;
			break;

		case JOB_SHAMAN:
			if (itemTable->dwAntiFlags & ITEM_ANTIFLAG_SHAMAN)
				return false;
			break;

		case JOB_SURA:
			if (itemTable->dwAntiFlags & ITEM_ANTIFLAG_SURA)
				return false;
			break;
#ifdef ENABLE_WOLFMAN_CHARACTER
		case JOB_WOLFMAN:
			if (itemTable->dwAntiFlags & ITEM_ANTIFLAG_WOLFMAN)
				return false;
			break;
#endif
	}
...
void CHARACTER::AddClMaterial(TItemPos tPos, BYTE bPos)
{
	...
	{
		bool bStop = false;
		if(pkItemMaterial[0]->GetType() == ITEM_WEAPON)
		{
		...
		else if(pkItemMaterial[0]->GetType() == ITEM_ARMOR)
		{
			if (pkItemMaterial[0]->GetOriginalVnum() == pkItem->GetOriginalVnum())
				bStop = true;
			else if(pkItem->GetType() != ITEM_ARMOR && pkItem->GetType() != ITEM_COSTUME)
				bStop = true;
		...
		else if(pkItemMaterial[0]->GetType() == ITEM_COSTUME)
		{
			if (pkItemMaterial[0]->GetOriginalVnum() == pkItem->GetOriginalVnum())
				bStop = true;
			else if(pkItem->GetType() != ITEM_ARMOR && pkItem->GetType() != ITEM_COSTUME)
			...
			{
				if (((IS_SET(pkItemMaterial[0]->GetAntiFlag(), ITEM_ANTIFLAG_FEMALE)) && (!IS_SET(pkItem->GetAntiFlag(), ITEM_ANTIFLAG_FEMALE))) || ((IS_SET(pkItemMaterial[0]->GetAntiFlag(), ITEM_ANTIFLAG_MALE)) && (!IS_SET(pkItem->GetAntiFlag(), ITEM_ANTIFLAG_MALE))))
					bStop = true;
			}
		}
		else
...
{
	if(CountSpecifyItem(39063) < 1){
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("[Gaya] Nu ai suficiente iteme Gaya Market."));
		return;
	}
	
	RemoveSpecifyItem(39063, 1);
...
void CHARACTER::GemShopAdd(BYTE bPos)
{
	...
	if(CountSpecifyItem(39064) < needCount[bPos]){
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("[Gaya] Nu ai suficiente : Gaya Market Expansion.Ai nevoie de mai multe %d pentru a debloca acest slot."), needCount[bPos] - CountSpecifyItem(39064));
		return;
	}

	if(m_gemItems[bPos].bSlotStatus == 0){
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("[Gaya] Nu se mai poate debloca un alt slot."));
		return;
	}
	
	RemoveSpecifyItem(39064, needCount[bPos]);
...
bool CHARACTER::CreateGaya(int glimmerstone_count, LPITEM metinstone_item, long long cost, int pct)
{
	if (!this)
		return false;
	...
	if (CountSpecifyItem(50926) < glimmerstone_count)
	{
	...
	RemoveSpecifyItem(50926, glimmerstone_count);
...
{
	std::auto_ptr<SQLMsg> pMsg(DBManager::instance().DirectQuery("SELECT monede FROM account.account WHERE id = '%d';", GetDesc()->GetAccountTable().id));
...
{
	DBManager::instance().DirectQuery("UPDATE account.account SET monede = '%d' WHERE id = '%d';", black, GetDesc()->GetAccountTable().id);
}
...
void CHARACTER::BuffSummon(LPITEM buffItem)
{
	CSupportSystem* buffSystem = GetSupportSystem();
	DWORD mobVnum = 0;
	
	if (GetMapIndex() == 113 || GetMapIndex() == 1 || GetMapIndex() == 21 || GetMapIndex() == 41)
		return;	
	
	if (!buffSystem || !buffItem)
		return;
				
	if(buffItem->GetValue(1) != 0)
		mobVnum = buffItem->GetValue(1);
	
	buffSystem->Summon(mobVnum, buffItem, false);
}

void CHARACTER::BuffUnsummon(LPITEM buffItem)
{
	CSupportSystem* buffSystem = GetSupportSystem();
	DWORD mobVnum = 0;
	...
	buffSystem->Unsummon(mobVnum);
}

void CHARACTER::CheckBuff()
{
	CSupportSystem* buffSystem = GetSupportSystem();
	LPITEM buffItem = GetWear(WEAR_NORMAL_BUFF);
	DWORD mobVnum = 0;
	...
	if(buffSystem->CountSummoned() == 0)
	{
		buffSystem->Summon(mobVnum, buffItem, false);
	}
}

bool CHARACTER::IsBuffSummon()
{
	return (GetWear(WEAR_NORMAL_BUFF));
}
#endif
