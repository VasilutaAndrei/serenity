...
int CalcAttBonus(LPCHARACTER pkAttacker, LPCHARACTER pkVictim, int iAtk)
{
	...
	{
		iAtk += (iAtk * pkAttacker->GetPoint(POINT_ATTBONUS_HUMAN)) / 100;
		iAtk += (iAtk * pkAttacker->GetPoint(POINT_RESIST_CLAW)) / 100;
...
int battle_hit(LPCHARACTER pkAttacker, LPCHARACTER pkVictim, int & iRetDam)
{
	...
	//PROF_UNIT puDam("Dam");
#ifdef ENABLE_ATTENDANCE_EVENT
	EDamageType damageType = (pkVictim->GetRaceNum() >= 6915 && pkVictim->GetRaceNum() <= 6919) ? DAMAGE_TYPE_SPECIAL : DAMAGE_TYPE_NORMAL;
...
DWORD GET_ATTACK_SPEED(LPCHARACTER ch)
{
	...
	/*if (item && item->GetSubType() == WEAPON_DAGGER)
		...
		real_speed /= 2;
	*/
		if (item && item->GetSubType() == WEAPON_DAGGER && !ch->IsRiding())
		{
			real_speed /= 2;
		}
		else if (item && item->GetSubType() == WEAPON_DAGGER && ch->IsRiding())
		{
			real_speed += 100;
		}
		else if (item && item->GetSubType() == WEAPON_CLAW && !ch->IsRiding())
		{
			real_speed /= 2;
		}
		/*else if (item && item->GetSubType() == WEAPON_CLAW && ch->IsRiding())
