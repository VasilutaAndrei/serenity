...
//Fix meley lair
void CGuildManager::DungeonStart(DWORD dwGuildID, DWORD dwDungeon_start)
{
	TPacketDGGuildDungeonST sPacket;
	sPacket.dwGuildID = dwGuildID;
	sPacket.dwDungeon_start = dwDungeon_start;
	CClientManager::instance().ForwardPacket(HEADER_DG_GUILD_DUNGEON_ST, &sPacket, sizeof(sPacket));
	
	char szBuf[512];
	snprintf(szBuf, sizeof(szBuf), "UPDATE guild%s SET dungeon_start = %u WHERE id = %u;", GetTablePostfix(), dwDungeon_start, dwGuildID);
	CDBManager::instance().AsyncQuery(szBuf);
}

void CGuildManager::DungeonStartRequest(DWORD dwDungeon_start)
{
	TPacketDGGuildDungeonST sPacket;
	sPacket.dwDungeon_start = dwDungeon_start;
	CClientManager::instance().ForwardPacket(HEADER_DG_GUILD_DUNGEON_ST, &sPacket, sizeof(sPacket));
	...
	char szBuf[512];
	snprintf(szBuf, sizeof(szBuf), "SELECT id%s WHERE dungeon_start = %u;", GetTablePostfix(), dwDungeon_start);
	CDBManager::instance().AsyncQuery(szBuf);
}
#endif
